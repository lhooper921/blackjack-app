    ...
    document.addEventListener('click', async function (event) {
        ...
        switch (event.target.id) {
            case 'bet-btn':
                ...
                if (bet > player.chips) {
                    alert("Error: Bet exceeds player chip count");
                    break;
                }
                handFinished = false;
                window.onbeforeunload =  () => {updateUser({chips: player.chips - bet, handPlayed: handFinished, id: USER.id});}
...
async function resolveHand(status) {
    ...
    //If a player busts off a double, use PLAYER BUST case
    let handStatus = status === 'PLAYER BUST DOUBLE' ? 'PLAYER BUST' : status;
...
function renderReport(result) {
    ...
    player.chips += payout;
    handFinished = true;
    updateUser({ chips: player.chips, handPlayed: handFinished, id: USER.id });
    $(".player-action").hide();
